...
namespace AltMediatR.Core.Mediator
{
    ...
    /// <para>
    /// When <see cref="ICompiledHandlerDispatcher"/> is registered (via <c>AddGeneratedHandlers()</c>)
    /// the Mediator routes requests through the compile-time-generated dispatcher — eliminating all
    /// <c>MakeGenericMethod</c> / <c>Invoke</c> reflection at runtime.  If no compiled dispatcher is
    /// registered the Mediator falls back to the cached-reflection path for full backward compatibility.
    /// </para>
    /// </summary>
    public class Mediator : IMediator, IPipelineDispatcher
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly ICompiledHandlerDispatcher? _compiledDispatcher;

        // Reflection-based fallback: cache the open-generic method and its closed variants to avoid
        // repeated GetMethod / MakeGenericMethod calls.  Only used when no ICompiledHandlerDispatcher
        // is registered.
        ...
        public Mediator(IServiceProvider serviceProvider)
        {
            ...
            // Resolve the compiled dispatcher emitted by AltMediatR.SourceGenerator (optional).
            _compiledDispatcher = serviceProvider.GetService<ICompiledHandlerDispatcher>();
        }

        // ------------------------------------------------------------------
        // IPipelineDispatcher — called by the generated GeneratedHandlerDispatcher
        // to run the full pipeline without reflection.
        // ------------------------------------------------------------------

        Task<TResponse> IPipelineDispatcher.ExecutePipelineAsync<TRequest, TResponse>(
            TRequest request, CancellationToken cancellationToken)
            => SendCoreAsync<TRequest, TResponse>(request, cancellationToken);

        Task IPipelineDispatcher.ExecuteVoidPipelineAsync<TRequest>(
            TRequest request, CancellationToken cancellationToken)
            => SendCoreVoidAsync<TRequest>(request, cancellationToken);
        ...
        public async Task<TResponse> SendAsync<TResponse>(IRequest<TResponse> request, CancellationToken cancellationToken = default)
        {
            ...
            // Use the compiled dispatcher generated by AltMediatR.SourceGenerator (zero reflection).
            if (_compiledDispatcher != null &&
                _compiledDispatcher.TryDispatch(this, request, cancellationToken, out var compiledTask))
                return await compiledTask!.ConfigureAwait(false);

            // Reflection-based fallback — used when AddHandlersFromAssembly is used instead of AddGeneratedHandlers.
        ...
        /// <summary>
        /// Sends a void request to its handler, executing:
        /// Pre-processors -> Pipeline Behaviors -> Handler -> Post-processors.
        /// </summary>
        /// <param name="request">The request instance.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        ...
        public async Task SendAsync(IRequest request, CancellationToken cancellationToken = default)
        {
            ...
            // Use the compiled dispatcher generated by AltMediatR.SourceGenerator (zero reflection).
            if (_compiledDispatcher != null &&
                _compiledDispatcher.TryDispatchVoid(this, request, cancellationToken, out var compiledTask))
            {
                await compiledTask!.ConfigureAwait(false);
                return;
            }
        ...
        /// <summary>
        /// True-void pipeline path: hides Unit from the public API but applies the same behaviors
        /// and processors internally via the Unit adapter.
        /// </summary>
